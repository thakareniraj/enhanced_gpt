...
function App() {
  ...
  const ALAN_Key = `aae908c68a98d7132e5c4641906dafb32e956eca572e1d8b807a3e2338fdd0dc/stage`
  ...
  alanBtn({
    ...
    onCommand: (commandData) => {
      if(commandData.command === 'name'){
        ...
        setInput(commandData["value"])
      }
      else if(commandData.command === 'feedback'){
...
const [currModel,setCurrModel] = useState("text-davinci-001");
...
async function editResponse(){
  const response = await fetch("http://localhost:3080/editResponse",{
  method : 'POST',
  headers : {
    'Content-Type': "application/json"
  },
...
await setChatLog([...newChatLog,{user : "gpt", message :`${data.message}`}])
    ...
    {Array.isArray(models) && models.map((model, index) => (
      ...
      <section className="chatsection">
        ...
        <form onSubmit = {handleSubmit}>
          <input className="chat-input-textarea" value={input} onChange={handleChange} rows='1' placeholder="Type your message here"></input>
          </form>
        </div>
      </section>
